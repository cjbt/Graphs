...
# Fill this out
traversalPath = []
# traversalPath = ['n', 'n', 's', 's', 's', 's',
#                  'n', 'n', 'w', 'w', 'e', 'e', 'e', 'e']


def dft(room, visited, randomDir):
    # dft
    stack = []
        ...
        if v not in visited:
            if player.currentRoom.id not in visited:
                ...
                for i in range(0, len(player.currentRoom.getExits())):
                    visited[v][player.currentRoom.getExits()[i]] = '?'

                if randomDir in player.currentRoom.getExits():
                    prev = player.currentRoom.id
                    # travels
                    player.travel(randomDir)
...
def traverse_path(room):
    ...
    visited[0] = {}
    for i in range(0, len(player.currentRoom.getExits())):
        visited[0][player.currentRoom.getExits()[i]] = '?'

    randomDir = None
    # pick random unexplosed direction - picks next one on the list
    for direction in visited[player.currentRoom.id]:
    ...
    # logs the direction
    visited[0][randomDir] = player.currentRoom.id
    traversalPath.append(randomDir)
